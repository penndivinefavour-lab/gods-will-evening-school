...
import { GET, POST } from '@/app/api/student-guardians/route'
import { GET as GET_BY_ID, PATCH } from '@/app/api/student-guardians/[id]/route'
...
function createGetRequest(search = ''): NextRequest {
  return new NextRequest(`http://localhost:3000/api/student-guardians${search ? `?${search}` : ''}`, {
    method: 'GET',
  })
}

function createPostRequest(body: Record<string, unknown>): NextRequest {
  return new NextRequest('http://localhost:3000/api/student-guardians', {
    method: 'POST',
...
function createPatchRequest(id: string, body: Record<string, unknown>): NextRequest {
  return new NextRequest(`http://localhost:3000/api/student-guardians/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(body),
  })
}
...
describe('GET /api/student-guardians', () => {
  ...
  it('returns student guardian relationships for authorized user', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({
        data: [{ student_guardian_id: '1', student_id: 's1', guardian_id: 'g1' }],
        count: 1,
        error: null,
      }) as any
    )
...
describe('POST /api/student-guardians', () => {
  it('rejects unauthenticated request', async () => {
    mockedRequirePermission.mockResolvedValue(null)
    const response = await POST(createPostRequest({ student_id: 's1', guardian_id: 'g1' }))
    expect(response.status).toBe(401)
  })
  ...
  it('rejects duplicate relationship', async () => {
    const dupResult = { data: { student_guardian_id: 'dup' }, error: null }
...
describe('GET /api/student-guardians/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockedRequirePermission.mockResolvedValue(roleContext)
  })

  it('returns student guardian relationship by id', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({ data: { student_guardian_id: '1', student_id: 's1', guardian_id: 'g1' }, error: null }) as any
    )
    const response = await GET_BY_ID(createGetRequest(), { params: Promise.resolve({ id: '1' }) })
    expect(response.status).toBe(200)
    const body = await response.json()
    expect(body.data.student_guardian_id).toBe('1')
  })
...
describe('PATCH /api/student-guardians/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockedRequirePermission.mockResolvedValue(roleContext)
  })

  it('updates student guardian relationship', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({ data: { student_guardian_id: '1', is_primary: true }, error: null }) as any
    )
    const response = await PATCH(createPatchRequest('1', { is_primary: true }), { params: Promise.resolve({ id: '1' }) })
    expect(response.status).toBe(200)
    const body = await response.json()
    expect(body.data.is_primary).toBe(true)
  })

  it('rejects cross-school update when relationship belongs to another school', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({ data: null, error: null }) as any
    )
    const response = await PATCH(createPatchRequest('1', { is_primary: true }), { params: Promise.resolve({ id: '1' }) })
