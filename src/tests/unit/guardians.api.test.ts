...
import { GET, POST } from '@/app/api/guardians/route'
import { GET as GET_BY_ID, PATCH } from '@/app/api/guardians/[id]/route'
...
function createGetRequest(search = ''): NextRequest {
  return new NextRequest(`http://localhost:3000/api/guardians${search ? `?${search}` : ''}`, {
    method: 'GET',
  })
}

function createPostRequest(body: Record<string, unknown>): NextRequest {
  return new NextRequest('http://localhost:3000/api/guardians', {
    method: 'POST',
...
function createPatchRequest(id: string, body: Record<string, unknown>): NextRequest {
  return new NextRequest(`http://localhost:3000/api/guardians/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(body),
  })
}
...
describe('GET /api/guardians', () => {
  ...
  it('returns guardians for authorized user', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({
        data: [{ guardian_id: '1', first_name: 'Jane', last_name: 'Doe' }],
        count: 1,
        error: null,
      }) as any
    )
...
describe('POST /api/guardians', () => {
  it('rejects unauthenticated request', async () => {
    mockedRequirePermission.mockResolvedValue(null)
    const response = await POST(createPostRequest({ first_name: 'Jane', last_name: 'Doe', phone: '123' }))
    expect(response.status).toBe(401)
  })
...
describe('GET /api/guardians/[id]', () => {
  ...
  it('returns guardian by id', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({ data: { guardian_id: '1', first_name: 'Jane', last_name: 'Doe' }, error: null }) as any
    )
    const response = await GET_BY_ID(createGetRequest(), { params: Promise.resolve({ id: '1' }) })
    expect(response.status).toBe(200)
    const body = await response.json()
    expect(body.data.guardian_id).toBe('1')
  })

  it('returns 404 when guardian not found', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({ data: null, error: null }) as any
    )
    const response = await GET_BY_ID(createGetRequest(), { params: Promise.resolve({ id: '1' }) })
...
describe('PATCH /api/guardians/[id]', () => {
  ...
  it('updates guardian', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({ data: { guardian_id: '1', first_name: 'Updated' }, error: null }) as any
    )
  ...
  it('rejects cross-school update when guardian belongs to another school', async () => {
    mockedGetSupabaseServerClient.mockResolvedValue(
      createSupabaseClient({ data: null, error: null }) as any
    )
    const response = await PATCH(createPatchRequest('1', { first_name: 'Updated' }), { params: Promise.resolve({ id: '1' }) })
